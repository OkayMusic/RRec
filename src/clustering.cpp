...
#define MAXCLUSTERSIZE 1000000
static std::vector<std::array<int, 2>> clusterPoints(MAXCLUSTERSIZE);
static std::vector<std::array<int, 2>> perimPoints(MAXCLUSTERSIZE);
static std::vector<std::array<int, 2>> corePoints(MAXCLUSTERSIZE);
...
class Cluster
{
  ...
  public:
    ...
    int size() { return corePoints.size() + outerPoints.size(); }
};
...
std::vector<Cluster> dbscan(std::vector<std::vector<bool>> &thresh,
                            std::vector<std::vector<int>> &pointFlags)
{
...
void showDBSCAN(cv::Mat threshold, cv::Mat outImage)
{
    /*
    ...
    */
    ...
    std::vector<Cluster> clusters{dbscan(thresh, pointFlags)};
